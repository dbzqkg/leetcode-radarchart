...
function buildRequest(cookie, payload) {
  return { method: 'POST', headers: { 'Content-Type': 'application/json', Cookie: cookie, Referer: 'https://leetcode.cn/', Origin: 'https://leetcode.cn', 'User-Agent': UA }, body: payload };
}
...
async function doAnalyze(cookie, send) {
  const tagMap = new Map();
  const cogRaw = [0,0,0,0,0,0];
  let totalQ = 0, loaded = 0, skip = 0, hasMore = true;

  while (hasMore) {
    const payload = JSON.stringify({ operationName: 'problemsetQuestionList', variables: { categorySlug: '', skip, limit: LIMIT, filters: { status: 'AC' } }, query: 'query problemsetQuestionList($categorySlug: String, $limit: Int, $skip: Int, $filters: QuestionListFilterInput) { problemsetQuestionList(categorySlug: $categorySlug, limit: $limit, skip: $skip, filters: $filters) { hasMore total questions { difficulty topicTags { name nameTranslated slug } } } }' });
    const resp = await fetch(API_URL, buildRequest(cookie, payload));
    if (resp.status !== 200) { send({ status: 'ERROR', message: `凭证失效或请求受限 [HTTP ${resp.status}]` }); return; }
    const json = await resp.json();
    if (json.errors) { send({ status: 'ERROR', message: `凭证失效或请求受限 [errors: ${JSON.stringify(json.errors)}]` }); return; }
    const ps = json.data.problemsetQuestionList;
    hasMore = ps.hasMore; totalQ = ps.total;
    for (const q of ps.questions) {
      loaded++;
      const d = q.difficulty, qScore = getDifficultyScore(d);
      for (const t of (q.topicTags || [])) {
        const name = (t.nameTranslated && t.nameTranslated !== 'null') ? t.nameTranslated : t.name;
        if (!name) continue;
        const slug = t.slug || '';
        let s = tagMap.get(name);
        if (!s) { s = { name, slug, total: 0, easy: 0, medium: 0, hard: 0 }; tagMap.set(name, s); }
        if (!s.slug) s.slug = slug;
        s.total++; if (d === 'EASY') s.easy++; else if (d === 'MEDIUM') s.medium++; else if (d === 'HARD') s.hard++;
        const w = TAG_WEIGHTS[slug] || DW;
        for (let i = 0; i < 6; i++) cogRaw[i] += qScore * w[i];
      }
    }
    send({ status: 'RUNNING', message: `正在统计已解题目... (${loaded}/${totalQ})`, currentStep: loaded, totalSteps: totalQ });
    skip += LIMIT;
    await new Promise(r => setTimeout(r, SLEEP_MS));
  }

  for (const s of tagMap.values()) {
    const score = s.easy * 1 + s.medium * 2.5 + s.hard * 5;
    if (score === 0) { s.mastery = 0; continue; }
    const b = TIER_BENCHMARKS[s.name] || 8;
    s.mastery = Math.round(1000 * (1 - Math.exp(-Math.log(2) / b * score))) / 10;
  }

  const cognitive = {};
  for (let d = 0; d < 6; d++) {
    const lambda = Math.log(2) / COG_BENCHMARKS[d];
    cognitive[d] = Math.round(1000 * (1 - Math.exp(-lambda * cogRaw[d]))) / 10;
  }

  const sorted = [...tagMap.values()].sort((a, b) => b.total - a.total);
  send({ status: 'SUCCESS', data: { total: totalQ, tags: sorted, cognitive } });
}
...
export async function onRequest(context) {
  ...
  context.waitUntil(
    doAnalyze(cookie, send).finally(() => writer.close())
  );
